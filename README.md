...
設定はツールディレクトリの`config.json`から読み込みます。ビルド済みバイナリでは実行バイナリと同じディレクトリ、`go run`ではカレントディレクトリがツールディレクトリです。`workspaceName`で成果物ディレクトリ名、`builtinAllowedCommands`で自動承認対象コマンド、`builtinAllowedPaths`でCopilotの自動承認対象パスを指定します。
