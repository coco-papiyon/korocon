...
担当者別の起動は`--implementer`（`-i`）または`--reviewer`（`-r`）で指定できます。`--implementer`は実装者が担当するIssue、コンフリクトPR、レビュー指摘修正PRを表示します。`--reviewer`はIssue/PRの種別選択を省略し、未レビューの`state:pr_created` PRだけを表示します。
