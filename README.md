...
起動直後にGitHubから取得する情報を`issue`または`pr`で選択します。`issue`を選んで番号を入力すると、Issue本文・ラベル・コメントを取得し、状態ラベルに応じて設計または実装をCodexへ自動投入します。`pr`を選ぶと状態・Draft・レビュー判定を含むPR一覧を表示し、指定したPRのレビューを開始します。この機能には認証済みのGitHub CLI (`gh`) が必要です。
...
PRレビュー完了後は、未入力Enterなどで承認、`/rerun [補足]`でレビュー再実行、それ以外の入力でレビュー指摘の修正を開始します。修正はPR head用worktreeで設計検討・実装・テストを行い、承認後にcommit・pushして再レビューします。レビュー承認後は動作確認へ進み、PRをCLOSEDまたはMERGEDにして未入力Enterまたは`/check`を入力すると完了し、最初の`issue`/`pr`選択へ戻ります。
