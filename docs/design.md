...
9. `state:pr_review_comment`などレビュー修正状態のPRを選択すると、実装者と`review-comment-fix`スキルで設計検討、実装、テストを行う
10. 修正結果を`review_fix_implementation/`へ保存し、承認後にcommit・PR headへpushして`state:review_fixed`へ更新する。再レビューは続けず実装者を停止して最初の選択へ戻る
11. `state:review_fixed`のPRを再選択すると、レビューアで再レビューする
12. 動作確認後、PRがCLOSEDまたはMERGEDなら`state:completed`へ更新して最初の選択へ戻る
