...
`--implementer`（`-i`）では実装者が担当するIssue一覧と、コンフリクトまたはレビュー指摘修正状態のPR一覧を表示します。`--reviewer`（`-r`）ではIssue/PR種別の選択を省略し、`state:pr_created`の未レビューPR一覧だけを表示します。実装者モードとレビューアモードはCLI引数でのみ指定し、各工程で使用するProviderとModelは既存の役割別設定に従います。
