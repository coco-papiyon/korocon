...
- ツールディレクトリの`config.json`から設定を読む（ビルド済みバイナリはバイナリ配置先、`go run`はカレントディレクトリ）
- 設計・実装成果物をkorobokcle互換のworkspace構造へ保存する
- 実装は専用worktree上で行い、実装用と検証用のCodexセッションを分離する
- 実装と検証を設定回数まで反復する
- Issue一覧にローカル工程状態を表示し、停止したIssueを設計または実装の自動処理対象へ戻せる
...
未コミット変更の同期方針は設定ファイルの`syncDirtyWorktree`を基本値とし、CLIの`-s`（`--sync-dirty`）で実行単位に`fail`または`stash`へ上書きできます。CLI指定は設定ファイルへ保存しません。
...
ツールディレクトリを解決し、そのディレクトリの`config.json`を読みます。ビルド済みバイナリではバイナリ配置先、`go run`ではカレントディレクトリをツールディレクトリとします。ファイルがない場合は`workspaceName: .workspace`を使用します。不正なJSON、未知の設定項目、パスとして解釈できる`workspaceName`は起動エラーです。状態DBの`korocon.db`も同じツールディレクトリへ配置します。

`korocon config init`は`baseBranch`、`branchNamePattern`、`startupCommand`を対話入力し、空入力を既定値として扱います。続けて共通のモデル設定処理を呼び出し、実装者・検証者・レビューアのProviderとModelを設定してから一度だけ保存します。モデル候補は各役割で選択されたProviderから解決し、Copilotは`auto`、`gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna`、`gpt-5-mini`、`claude-sonnet-4.6`、`claude-opus-4.6`を表示します。Copilotの既定値は`auto`で、具体的なモデル名は直接入力も許可します。ProviderをCodexからCopilotへ変更した場合、空入力時のModel既定値は`auto`です。既存ファイルは`--force`なしでは上書きしません。`korocon config model`は同じモデル設定処理を既存設定へ適用します。
...
実装設定として、ブランチ名規則、worktree親ディレクトリ、実装・検証ループ回数、自動処理の再取得間隔、PRのbaseブランチ、動作確認の有効フラグ`runtimeVerificationEnabled`、動作確認用の`startupCommand`も保持します。`runtimeVerificationEnabled`の既定値は`true`です。`builtinAllowedCommands`はコマンド許可、`builtinAllowedPaths`はCopilotのパス・diff許可として独立して保持します。
...
AIからコマンド実行の承認要求を受信すると許可リストを判定し、安全な一致なら即座に承認します。それ以外は内容を画面へ表示し、未入力Enter、`/approve`、`/allow`、`/allow-job`、`/allow-process`、`/decline`の入力を待ってProviderへ応答します。`/allow`は要求コマンドをアプリの判断で承認し、`/allow-job`は現在のジョブ中、`/allow-process`は現在のkoroconプロセス中のすべてのコマンド実行要求を一時許可します。`/allow <command>`は指定コマンドを恒久的な許可リストへ追加します。CopilotのACP `session/request_permission`はこの共通形式へ変換します。

複数の手動承認要求を同時に受信した場合は到着順に直列化し、現在の要求へ応答した後に次の承認メッセージと入力プロンプトを表示します。承認待ちが存在することを理由に後続要求を拒否しません。

`&&`、`||`、パイプで連結されたコマンドは引用符とエスケープを考慮して分割し、全要素が`builtinAllowedCommands`に一致する場合だけ自動承認します。`;`、コマンド置換、`2>&1`と`2>/dev/null`以外のリダイレクトは拒否します。

`builtinAllowedCommands`に複合スクリプト全文が明示されている場合は、空白を正規化した完全一致を分割判定より先に確認します。完全一致したエントリだけは複数行、括弧、コマンド置換を許可し、接頭辞一致による引数追加は許可しません。

Issue実装・検証用Copilotセッションは、確定した実装worktreeをセッション固有の暗黙的な許可ルートとして保持します。`path`、`fileName`およびdiffの全対象を正規化し、許可ルート配下に収まる場合だけ共通の手動承認処理を省略します。この許可は通常セッションや別worktreeへ引き継ぎません。
...
Markdown成果物を生成するすべてのAIプロンプトには共通の出力契約を末尾へ付加します。対象はIssueの設計・再設計・実装、PRのレビュー・再レビュー・レビュー指摘修正・コンフリクト解消・動作確認です。スキルがリポジトリ内のファイルへ成果物を保存した場合も、最終回答へ同じMarkdown全文を再掲させ、koroconが最終回答を保存しても本文が欠落しないようにします。実装・修正の合否判定に使う検証者プロンプトはJSONオブジェクトのみを要求するため対象外です。
...
6. レビュー承認時は`runtimeVerificationEnabled`が`true`ならPR head用worktreeを再度最新化し、レビューアへ同worktreeでの動作確認を指示する。`startupCommand`があれば同worktreeで追加起動する。フラグが`false`なら動作確認を省略してPR処理を終了する。`/rerun`は同じworktreeを最新化してレビューを再実行する
...
許可リストに一致しない`requestApproval`はターン処理を待機させます。未入力Enterまたは`/approve`は`accept`、`/decline`は`decline`をJSON応答として返します。`/allow`は要求コマンドをアプリの判断で`accept`し、`/allow-job`と`/allow-process`はそれぞれジョブ内またはプロセス内の全コマンド要求を一時的に自動承認します。`/allow <command>`は指定コマンドを恒久的な許可リストへ追加して`accept`を返します。未対応形式のサーバー要求は自動承認せず、エラー応答します。
