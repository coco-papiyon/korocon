...
AI設定は実装者、検証者、レビューアごとにProviderとModelを保持します。検証者・レビューアの各未指定値は実装者の解決済み設定を継承し、CLI引数、設定ファイル、既定値の順で解決します。Issue設計・実装とレビュー指摘修正は実装者、実装検証は検証者、PRレビューはレビューアを使用します。
...
1. `gh pr list --state all --json`でPRを取得し、状態、Draft、レビュー判定を表示する
2. 入力されたPR番号から本文、head/baseブランチ、コメント、レビューを取得する
3. `review-pull-request`スキルを使ってレビューし、`review/`へ成果物を保存する
4. 承認は`state:review_approved`として動作確認へ進み、`/rerun`は同じレビューを再実行する
5. レビュー修正指示はPRへコメントし、PR headのworktreeで`review-comment-fix`スキルを使って設計検討・実装・テストを行う
6. 修正結果を`review_fix_implementation/`へ保存し、承認後にcommit・PR headへのpushを行って再レビューする
7. 動作確認後、PRがCLOSEDまたはMERGEDなら`state:completed`へ更新して最初の選択へ戻る
