...
実装設定として、ブランチ名規則、worktree親ディレクトリ、実装・検証ループ回数も保持します。既定値は`issue_#<issue番号>`、`../`、3回です。

### `internal/daemon`

端末入力、`/model`などの内部コマンド、ターンキュー、結果表示を管理します。入力は受け付け時にジョブIDを採番し、Codex用の単一workerが順番に処理します。

Codexから承認要求を受信すると内容を画面へ表示し、`/approve`または`/decline`の入力を待ってapp-serverへ応答します。
...
6. ジョブ開始前にGitHubの状態ラベルをrunningへ更新する
7. workerが`turn/start`をCodexの標準入力へ送る
8. JSONL通知をログへ記録し、`turn/completed`まで待つ
9. 最終`agentMessage`と使用トークン数を画面へ表示する
10. 成果物を`<workspaceName>/<工程>/<番号>_<タイトル>.md`へ保存する
11. 正常完了時はready、失敗時はfailedへIssueラベルを更新する
12. Issue処理の正常完了後は保存先、承認または修正指示の案内と`> `を表示する
13. 設計承認時は設計用Codexを停止し、実装worktreeを準備する
14. 実装用・検証用Codexを起動し、実装と検証を設定回数まで反復する
15. 検証合格時は実装成果物を保存して承認待ちへ進む
16. 実装承認時は2つのCodexを停止する
17. 修正指示なら同じ工程の再実行をキューへ追加する
18. 終了時にContextをキャンセルし、すべてのCodexプロセスを停止する
...
Codexが`requestApproval`を送信するとターン処理は待機し、利用者の`/approve`または`/decline`をJSON応答として標準入力へ返します。未対応形式のサーバー要求は自動承認せず、エラー応答します。
