...
`Session`がCodex app-server、`CopilotSession`がCopilot ACPプロセスとのJSONL通信を所有します。`AgentSession`は役割ごとのProvider差を吸収し、どちらもプロセスと会話を複数ターンで再利用します。Copilotは`initialize`、`session/new`の後に`/ide`を`session/prompt`として送ります。
...
4. Codexは`thread/start`、Copilotは`session/new`を実行し、Copilotでは続けて`/ide`を送る
...
9. workerがProviderのターン開始要求を標準入力へ送る
...
- `runner.CopilotSession`: 1プロセス再利用、`/ide`初期化、モデル変更、承認変換を偽ACPサーバーで検証
