...
異なる役割の工程は同じAIプロセス内で続けて実行しません。PRレビューで修正指示を登録した場合はレビューアを停止して最初のIssue/PR選択へ戻り、次に同じPRを選択した時点で実装者による修正を開始します。レビュー指摘修正を承認した場合も実装者を停止して選択へ戻り、次回選択時にレビューアで再レビューします。コンフリクト解消の承認後も同様に選択へ戻ります。これにより、各工程では設定された担当者のProvider、Model、独立したAIセッションだけを使用します。
...
7. レビュー修正指示はPRへコメントして`state:pr_review_comment`へ更新し、修正を続けずレビューアを停止して最初の選択へ戻る
