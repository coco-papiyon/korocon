...
`--auto`を指定すると、フィルタ結果を番号の降順で再取得しながら先頭から順に処理します。処理完了後も選択入力へ戻らず、次の対象を自動選択します。実装者モードではIssueを優先し、対象Issueがなくなった後にPRを処理します。レビューアモードではPRだけを処理します。対象がない場合は`autoPollingInterval`だけ待機し、GitHub一覧とProject情報を再取得します。待機中のCtrl+Cは直ちにCLIを終了します。設計・実装・レビュー結果などの承認入力は自動化せず、従来どおり利用者の入力を待ちます。
