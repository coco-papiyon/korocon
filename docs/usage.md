...
レビュー結果の`## 結果`が`要修正`または`コメントあり`の場合は、レビュー結果をPRコメントへ登録して`state:pr_review_comment`へ更新し、入力を待たずにレビューアを停止して最初のIssue/PR選択へ戻ります。`問題なし`または判定できない形式の場合は、次の確認入力を受け付けます。
