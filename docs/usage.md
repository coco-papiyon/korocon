...
設計、実装、PRレビュー、再レビュー、レビュー指摘修正、コンフリクト解消、PR動作確認など、Markdown成果物を保存する工程では、スキルが別ファイルへ成果物を書き出した場合も最終回答に同じMarkdown全文を再掲するよう共通指示を付けます。koroconはAIの最終回答を正式な成果物として保存するため、「保存しました」「保存先: ...」だけの応答による成果物本文の欠落を抑止します。実装内容を機械判定する検証者の応答は従来どおりJSONのみとし、この共通指示の対象外です。
