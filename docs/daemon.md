...
起動時の選択は`ISSUE/PR`（大小文字非依存、`I`/`P`、空入力はIssue）で行います。PRがない場合やIssueがOpenでない場合は選択へ戻ります。`state:design_ready`または`state:implementation_ready`を選んだ場合は、保存済み成果物から承認待ち状態を復元し、AIジョブを自動投入しません。
