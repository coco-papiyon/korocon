...
func TestSelectGitHubInformationAcceptsEmptyAsIssue(t *testing.T) {
	old := loadIssue
	loadIssue = func(_ context.Context, _ string, number int, workspace string) (*issueworkflow.Workflow, error) {
		return &issueworkflow.Workflow{Issue: issueworkflow.Issue{Number: number, State: "OPEN"}, Phase: issueworkflow.PhaseDesign}, nil
	}
	defer func() { loadIssue = old }()
	var out bytes.Buffer
	_, selected, _, err := selectGitHubInformation(context.Background(), bytes.NewBufferString("\n42\n"), &out, ".", ".workspace")
...
func TestSelectGitHubInformationRetriesAfterClosedIssue(t *testing.T) {
	...
	defer func() { loadIssue = old }()
	var out bytes.Buffer
	_, selected, _, err := selectGitHubInformation(context.Background(), bytes.NewBufferString("issue\n1\nissue\n2\n"), &out, ".", ".workspace")
...
func TestSelectGitHubInformationRetriesAfterInvalidIssueNumber(t *testing.T) {
	original := loadIssue
	...
	var out bytes.Buffer
	_, selected, _, err := selectGitHubInformation(context.Background(), bytes.NewBufferString("issue\nabc\nissue\n3\n"), &out, ".", ".workspace")
	if err != nil || selected == nil || selected.Issue.Number != 3 {
		t.Fatalf("selected=%+v err=%v output=%q", selected, err, out.String())
	}
	if !strings.Contains(out.String(), "issue番号が不正です") || !strings.Contains(out.String(), "再入力してください") {
		t.Fatalf("invalid input message missing: %q", out.String())
	}
}
